...
class Command {
	...
	exec(){
		...
		_$child_processφ.spawnSync(("" + _$ImbaRunnerφ.default.instance().slice(0,-1)),this.args,{
			stdio: 'inherit',
			cwd: process.cwd()
		});
	...
	createFallbackScript(){
		...
		if (!(this.args[0].endsWith('.imba'))) {
			...
			fallbackScript = _$pathφ.join(process.cwd(),("." + (this.args[0]) + ".imba"));
			
			try {
				
				_$fsφ2.copyFileSync(sourceScript,fallbackScript);
				this.args[0] = ("." + (this.args[0]) + ".imba");
			} catch (φ) {
